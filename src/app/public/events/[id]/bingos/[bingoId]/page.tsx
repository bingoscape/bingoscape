...
export async function generateMetadata(
  ...
  if (!event) {
    return {
      ...
      description:
        "The requested bingo board could not be found or is not publicly available.",
    }
  }
  ...
  return {
    ...
    description:
      bingo.description ??
      `View this bingo board from the ${event.title} event`,
    openGraph: {
...
export default async function PublicBingoPage(props: {
  ...
  const prevBingoId =
    currentIndex > 0 ? bingos[currentIndex - 1]!.id : undefined
  const nextBingoId =
    currentIndex < bingos.length - 1 ? bingos[currentIndex + 1]!.id : undefined
          ...
          {bingos.length > 1 && (
            <div className="flex gap-2">
              {bingos.map((b, index) => (
                <Link
                  key={b.id}
                  href={`/public/events/${params.id}/bingos/${b.id}`}
                  passHref
                >
                  <Button
