...
export default async function BingoStatsPage(props: {
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {bingoStats.totalPossibleXP > 0
                ? (
                    (bingoStats.teamPoints.reduce(
                      (sum, team) => sum + team.xp,
                      0
                    ) /
                      bingoStats.totalPossibleXP) *
                    100
                  ).toFixed(1)
                : 0}
              %
      ...
      {hasItemStats ? (
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              No item goal submissions have been approved yet for this bingo
              board
            </CardDescription>
          </CardHeader>
          <CardContent>
            <p className="text-muted-foreground">
              Item statistics will appear here once teams start submitting and
              getting approved for item-based goals.
            </p>
          </CardContent>
        </Card>
      )}
