...
interface BingoSubmissionsContentProps {
  eventId: string
  bingoId: string
  initialBingo: BingoData
  initialTeams: Team[]
  userRole: "participant" | "management" | "admin" | null
}

export function BingoSubmissionsContent({
  eventId,
  bingoId,
  initialBingo,
  initialTeams,
  userRole,
}: BingoSubmissionsContentProps) {
  const router = useRouter()
  const [loading, setLoading] = useState(false)
  const [bingo] = useState<BingoData>(initialBingo)
  const [teams] = useState<Team[]>(initialTeams)
  const [selectedTeamIds, setSelectedTeamIds] = useState<string[]>([])
  const [tiles] = useState<TileData[]>(initialBingo.tiles || [])
