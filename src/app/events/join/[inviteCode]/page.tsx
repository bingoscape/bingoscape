...
export default function JoinEventPage(props: {
  ...
  useEffect(() => {
    ...
    if (status === "unauthenticated") {
      ...
      signIn(undefined, { callbackUrl }).catch((err) =>
  ...
  useEffect(() => {
    ...
    checkEventAndRegistration().catch((err) =>
      console.error("Failed to check event and registration", err)
    )
    ...
    <div className="container mx-auto py-10">
      <Card className="mx-auto max-w-md">
        <CardHeader>
          ...
          <CardDescription>
            {event
              ? `You've been invited to join ${event.title}!`
              : "You've been invited to join an event!"}
          </CardDescription>
        </CardHeader>
        <CardContent>
          <p>
            Click the button below to accept the invitation and join the event.
          </p>
        </CardContent>
        <CardFooter>
          <Button
