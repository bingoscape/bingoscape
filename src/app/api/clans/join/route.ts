...
export async function POST(req: Request) {
  ...
  if (invite.maxUses !== null && invite.currentUses >= invite.maxUses) {
    return NextResponse.json(
      { success: false, error: "Invite usage limit reached" },
      { status: 400 }
    )
  }
  ...
  const existingMembership = userClans.find(
    (clan) => clan.clanId === invite.clanId
  )

  if (existingMembership) {
    return NextResponse.json(
      { success: false, error: "Already a member of this clan" },
      { status: 400 }
    )
  }
