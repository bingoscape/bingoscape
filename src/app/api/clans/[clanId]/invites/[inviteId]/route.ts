...
export async function PATCH(
  ...
  try {
    ...
    const result = await updateClanInviteLabel({
    ...
    if (!result?.data?.success)
      return NextResponse.json(
        { error: result?.data?.error ?? "Failed to update invite" },
        { status: 400 }
      )
...
export async function DELETE(
  ...
  try {
    ...
    if (!result?.data?.success)
      return NextResponse.json(
        { error: result?.data?.error ?? "Failed to delete invite" },
        { status: 400 }
      )
...
export async function POST(
  ...
  if (action === "revoke") {
    try {
      ...
      if (!result?.data?.success)
        return NextResponse.json(
          { error: result?.data?.error ?? "Failed to revoke invite" },
          { status: 400 }
        )
