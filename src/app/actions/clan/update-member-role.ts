...
export const updateMemberRole = authActionClient
  .schema(schema)
  .action(async ({ parsedInput: { clanId, memberId, newRole }, ctx: { user } }) => {
    try {
      ...
      if (!currentMembership || !["admin", "management"].includes(currentMembership.role)) {
        return { success: false as const, error: "You don't have permission to update member roles" }
      }
      ...
      return { success: false as const, error: error instanceof Error ? error.message : "Failed to update member role" }
    }
  })
