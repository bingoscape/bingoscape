...
export async function leaveClan(clanId: string) {
  ...
  await db.delete(clanMembers).where(and(eq(clanMembers.clanId, clanId), eq(clanMembers.userId, session.user.id)))
...
export async function updateMemberRole(
  ...
  await db
    .update(clanMembers)
    .set({ role: newRole })
    .where(and(eq(clanMembers.clanId, clanId), eq(clanMembers.userId, memberId)))
}
