...
export async function deleteEvent(eventId: string) {
  ...
  try {
    ...
    // Delete the event and all related data in a transaction
    await db.transaction(async (tx) => {
      ...
      // Delete event buy-ins (need to get participant IDs first)
      ...
      if (participantIds.length > 0) {
        await tx.delete(eventBuyIns).where(eventBuyIns.eventParticipantId.in(participantIds.map((p) => p.id)))
      }

      // Get teams for this event
      const eventTeams = await tx.select({ id: teams.id }).from(teams).where(eq(teams.eventId, eventId))

      if (eventTeams.length > 0) {
        // Delete team members
        await tx.delete(teamMembers).where(teamMembers.teamId.in(eventTeams.map((t) => t.id)))

        // Delete teams
        await tx.delete(teams).where(eq(teams.eventId, eventId))
      }

      // Get bingos for this event
      const eventBingos = await tx.select({ id: bingos.id }).from(bingos).where(eq(bingos.eventId, eventId))

      if (eventBingos.length > 0) {
        // Get tiles for these bingos
        const bingoTiles = await tx
          .select({ id: tiles.id })
          .from(tiles)
          .where(tiles.bingoId.in(eventBingos.map((b) => b.id)))

        if (bingoTiles.length > 0) {
          // Delete tiles
          await tx.delete(tiles).where(tiles.bingoId.in(eventBingos.map((b) => b.id)))
        }
