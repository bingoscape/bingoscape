...
function getUserAgent(): string {
  const contactEmail =
    process.env.OSRS_WIKI_CONTACT_EMAIL ?? "unknown@example.com"
...
async function fetchAllGEPrices(): Promise<Map<number, CachedPrice>> {
  try {
    ...
    logger.error(
      { error, action: "fetchAllGEPrices" },
      "Error fetching GE prices"
    )
    throw new Error(
      `Failed to fetch GE prices: ${error instanceof Error ? error.message : "Unknown error"}`
    )
  }
}
 ...
 */
export async function getPricesForItems(
  itemIds: number[]
): Promise<Map<number, CachedPrice>> {
