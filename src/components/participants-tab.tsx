...
import { AddClanMembersDialog } from "@/components/events/add-clan-members-dialog"
...
export function ParticipantsTab({
  ...
  const [participantForDonations, setParticipantForDonations] =
    useState<Participant | null>(null)
  const [addClanMembersDialogOpen, setAddClanMembersDialogOpen] =
    useState(false)
  ...
  const handleClanMembersAdded = async () => {
    // Refresh participants list after clan members are added
    try {
      const updatedParticipants = await getEventParticipants(eventId)
      setParticipants(updatedParticipants)
      onParticipantsChange(updatedParticipants)
  ...
  return (
    <div className="flex flex-col gap-6">
      {/* Add Clan Members button */}
      {clanId &&
        (currentUserRole === "admin" || currentUserRole === "management") && (
          <div className="flex items-center justify-end">
            <Button
              variant="default"
              size="sm"
              onClick={() => setAddClanMembersDialogOpen(true)}
            >
      ...
      <AddClanMembersDialog
        eventId={eventId}
        clanId={clanId}
        open={addClanMembersDialogOpen}
        onOpenChange={setAddClanMembersDialogOpen}
        onSuccess={handleClanMembersAdded}
      />
    </div>
  )
}
