...
export function EventCommandPalette({
  ...
  const actions: EventAction[] = [
    // Participant Management
    {
      ...
      badge:
        pendingRegistrationsCount > 0 ? pendingRegistrationsCount : undefined,
  ...
  const orderedCategories = (
    ["participants", "boards", "collaboration", "configuration"] as const
  ).filter((cat) => (groupedActions[cat]?.length ?? 0) > 0)
        ...
        <DialogContent className="gap-0 p-0 sm:max-w-[600px]">
          ...
          <div className="relative flex items-center border-b bg-background">
            ...
            <Input
              ...
              className="h-12 border-0 pl-11 pr-20 text-sm focus-visible:ring-0 focus-visible:ring-offset-0 sm:h-14 sm:text-base"
              autoFocus
            />
            {search && (
              <Button
                    ...
                    <div key={category}>
                      ...
                      {!search && (
                        <div className="mb-1 px-3 py-2">
                          <h3
                            className={`text-xs font-semibold uppercase tracking-wider ${metadata.color}`}
                          >
                            {metadata.label}
                          </h3>
                        </div>
