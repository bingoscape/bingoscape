...
export function ItemTimelineCharts({
  ...
  if (teamTimelineComparison.length > 0) {
    const allDates = [
      ...new Set(teamTimelineComparison.map((t) => t.date)),
    ].sort()
      ...
      {dailyValueTimeline.length > 0 && (
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Cumulative and daily item value over time
            </CardDescription>
          </CardHeader>
          <CardContent>
            <ChartContainer
                ...
                >
                  ...
                  <ChartTooltip
                    content={({ active, payload }) => {
                      if (active && payload?.[0]) {
                        const data = payload[0].payload as DailyValueData & {
                          date: string
                        }
                        return (
                            ...
                            <div className="grid gap-2">
                              ...
                              <div className="flex items-center justify-between gap-2">
                                ...
                                <span className="text-sm font-bold">
                                  {formatGPValue(data.cumulativeValue)} GP
                                </span>
                              </div>
                              <div className="flex items-center justify-between gap-2">
                                <span className="text-xs text-muted-foreground">
                                ...
                                <span className="text-sm font-bold">
                                  {formatGPValue(data.dailyValue)} GP
                                </span>
                              </div>
                              <div className="flex items-center justify-between gap-2">
                                <span className="text-xs text-muted-foreground">
      ...
      {unifiedTeamTimeline.length > 0 && (
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Cumulative value progression by team over time
            </CardDescription>
          </CardHeader>
          <CardContent>
            <ChartContainer config={{}} className="h-[350px]">
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={unifiedTeamTimeline}>
                  ...
                  />
                  {Array.from(teamTimelineByTeam.keys()).map(
                    (teamId, index) => (
                      <Line
                        key={teamId}
                        type="monotone"
        ...
        {itemDiversityByTeam.length > 0 && (
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Unique items vs total submissions
              </CardDescription>
            </CardHeader>
            <CardContent>
              <ChartContainer
              ...
              >
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={itemDiversityByTeam} layout="horizontal">
                    <CartesianGrid strokeDasharray="3 3" />
                    <XAxis
                      type="number"
                      domain={[0, 1]}
        ...
        {efficiencyTrends.length > 0 && (
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Average value per submission over time
              </CardDescription>
            </CardHeader>
            <CardContent>
              <ChartContainer
                ...
                <ResponsiveContainer width="100%" height="100%">
                  ...
                  >
                    ...
                    <ChartTooltip
                      content={({ active, payload }) => {
                        if (active && payload?.[0]) {
                          const data = payload[0].payload as EfficiencyTrend & {
                            date: string
                          }
                          return (
                            <div className="rounded-lg border bg-background p-2 shadow-xs">
                              <div className="grid gap-2">
                                ...
                                <div className="flex items-center justify-between gap-2">
                                  ...
                                  <span className="text-sm font-bold">
                                    {formatGPValue(
                                      data.averageValuePerSubmission
                                    )}{" "}
                                    GP
                                  </span>
                                </div>
      ...
      {userStreaks.length > 0 && (
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Users with the longest consecutive day streaks
            </CardDescription>
          </CardHeader>
          <CardContent>
                ...
                >
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <div className="text-right">
                      ...
                      <p className="text-sm font-medium">
                        {formatGPValue(user.averageValuePerDay)} GP
                      </p>
                    </div>
                  </div>
                </div>
