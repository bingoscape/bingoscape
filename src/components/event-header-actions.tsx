  ...
  return (
    <div className="flex flex-wrap items-center justify-end gap-3">
      ...
      <BingoImportExportModal
        eventId={event.id}
        bingoId={
          (event.bingos?.length ?? 0) > 0 ? event.bingos![0]?.id : undefined
        }
        bingoTitle={
          (event.bingos?.length ?? 0) > 0 ? event.bingos![0]?.title : undefined
        }
            ...
            >
              <Link href={`/events/${eventId}/participants?tab=registrations`}>
                ...
                {pendingRegistrationsCount > 0 && (
                  <span className="flex h-5 min-w-5 items-center justify-center rounded-full bg-amber-500 px-1 text-xs font-medium text-white">
                    {pendingRegistrationsCount}
                  </span>
                )}
              </Link>
      ...
      <EventCommandPalette
        ...
        discordWebhooks={<DiscordWebhookManagement eventId={event.id} />}
        deleteEventModal={
          isAdmin ? (
            <Button
              variant="ghost"
