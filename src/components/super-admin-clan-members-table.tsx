...
export function SuperAdminClanMembersTable({
  members,
  clanId,
  ownerId,
}: SuperAdminClanMembersTableProps) {
    ...
    if (search) {
      ...
      filtered = filtered.filter(
        (member) =>
          ...
          (member.user.runescapeName?.toLowerCase().includes(searchLower) ??
            false)
      )
    }
    ...
    <div className="space-y-4">
      ...
      <div className="rounded-md border">
        <Table>
          <TableHeader>
            <TableRow>
              <SortableHeader column="username" onSort={handleSort}>
                User
              </SortableHeader>
              <SortableHeader column="role" onSort={handleSort}>
                Role
              </SortableHeader>
              <SortableHeader column="joinedAt" onSort={handleSort}>
                Joined
              </SortableHeader>
              <SortableHeader column="lastSeen" onSort={handleSort}>
                Last Seen
              </SortableHeader>
              <SortableHeader column="submissions" onSort={handleSort}>
                Submissions
              </SortableHeader>
                ...
                return (
                  <TableRow key={member.id}>
                    ...
                    <TableCell>
                      <div className="text-sm">
                        {member.lastSeen
                          ? formatDistanceToNow(new Date(member.lastSeen), {
                              addSuffix: true,
                            })
                          : "Never"}
                      </div>
